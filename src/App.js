...
function App(props) {
        ...
        selector: setupWalletSelector({
          ...
          modules: [
            ...
            setupFastAuth({
              networkId: NetworkId,
              signInContractId,
              relayerUrl: NetworkId === 'testnet' ? 'http://34.70.226.83:3030/relay' : 'https://near-relayer-mainnet.api.pagoda.co/relay',
            }),
            setupKeypom({
  ...
  useEffect(() => {
    setAvailableStorage(account.storageBalance ? Big(account.storageBalance.available).div(utils.StorageCostPerByte) : Big(0));
  }, [account]);
  ...
  if (signedIn) {
    localStorage.setItem('accountId', signedAccountId);
  }
        ...
        <BrowserRouter basename={process.env.PUBLIC_URL}>
          <Switch>
            ...
            <Route
              path='/consensus'
              component={() => {
                window.location.replace('https://nearconsensus2023.splashthat.com/');
                return null;
              }}
            />
            <Route
              path='/pitch'
              component={() => {
                window.location.replace('https://nearpitchfestconsensus.splashthat.com/');
                return null;
              }}
            />
