...
export const getCorrectAccessKey = async (
  userName,
  firstKeyPair,
  secondKeyPair
) => {
  ...
  const firstPublicKeyB58 =
    "ed25519:" + base_encode(firstKeyPair.getPublicKey().data);
  const secondPublicKeyB58 =
    "ed25519:" + base_encode(secondKeyPair.getPublicKey().data);

  const accessKey = accessKeys.find(
    (accessKey) =>
      accessKey.public_key === firstPublicKeyB58 || secondPublicKeyB58
  );
  if (!accessKey) {
...
export const handleCreateAccount = async (accountId, email, isRecovery) => {
  ...
  if (!!publicKey) {
    ...
    window.localStorage.setItem(
      ...
      JSON.stringify(accountDataStash)
    );
